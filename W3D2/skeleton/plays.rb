...
class Play
  ...
  def self.find_by_title(title)
    ...
    data.map { |datum| Play.new(datum) }[0]
  end
...
class Playwright
  ...
  def self.find_by_name(playwright)
    data = PlayDBConnection.instance.execute("SELECT * FROM playwrights WHERE name = '#{playwright}'")
    data.map { |datum| Playwright.new(datum) }[0]
  end
