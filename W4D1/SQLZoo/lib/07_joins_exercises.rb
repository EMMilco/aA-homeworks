...
def travoltas_busiest_years
  ...
  execute(<<-SQL)
  select movies.yr, COUNT(movies.id)
  from movies
  join castings on castings.movie_id = movies.id
  join actors on actors.id = castings.actor_id
...
def andrews_films_and_leads
  ...
  join actors as julie on julie_castings.actor_id = julie.id
...
def films_by_cast_size
  ...
  execute(<<-SQL)
    select movies.title, COUNT(castings.ord)
    from movies
    join castings on castings.movie_id = movies.id
    where movies.yr = 1978
    group by movies.id
    order by COUNT(castings.ord) desc, movies.title
  SQL
end
...
def colleagues_of_garfunkel
  ...
  execute(<<-SQL)
    ...
    from actors
    join castings on actors.id = castings.actor_id
    join castings as garfunkel on castings.movie_id = garfunkel.movie_id
    join actors as art on garfunkel.actor_id = art.id
    where art.name = 'Art Garfunkel'
