...
def larger_than_russia
  ...
  execute(<<-SQL)
    ...
    FROM countries
    JOIN countries as russia
    ON countries.population > russia.population
    WHERE russia.name = 'Russia'
  SQL
end
...
def richer_than_england
  ...
  SELECT countries.name
  FROM countries
  JOIN countries as uk
  ON countries.gdp / countries.population > uk.gdp / uk.population
...
def neighbors_of_certain_b_countries
  ...
  # 'Belize', 'Belgium'.
  execute(<<-SQL)
    SELECT DISTINCT countries.name, countries.continent
    FROM countries
    JOIN countries as b
    ON countries.continent = b.continent
...
def population_constraint
  ...
  execute(<<-SQL)
    ...
    FROM countries
    JOIN countries as canada
    ON countries.population > canada.population
    JOIN countries as poland
    ON countries.population < poland.population
    WHERE canada.name = 'Canada'
    AND poland.name = 'Poland'
  SQL
end
...
def sparse_continents
  ...
  execute(<<-SQL)
    ...
    WHERE countries.continent IN (
      ...
      HAVING MAX(countries.population) < 25000000
    )
  SQL
end
