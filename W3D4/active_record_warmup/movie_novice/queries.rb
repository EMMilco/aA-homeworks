...
def below_average_years
  ...
  Movie.select('count(*) as bad_movies, yr').group(:yr).order("bad_movies DESC").where('score < 5')
end
...
def pulp_fiction_actors
  ...
  Actor.joins(:movies).select('actors.name, actors.id').where("movies.title like 'Pulp Fiction'")
end
...
def uma_movies
  ...
  Movie.select(:id, :title, :yr).joins(:actors).where("actors.name like 'Uma Thurman'").order(yr: :desc)
end
